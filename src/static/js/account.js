...
async function setAccountMenu() {
    ...
    setTimeout( async () => {
        ...
        if (await isAuthenticated()) {
            ...
            accountMenu.innerHTML += `<button id="account-login-button" data-translate="account_login_button" class="jetbrains-mono-br">${dataLanguage["account_login_button"] || "Авторизація"}</button>`;
            accountMenu.innerHTML += `<button id="account-register-button" data-translate="account_register_button" class="jetbrains-mono-br">${dataLanguage["account_register_button"] || "Реєстрація"}</button>`;
